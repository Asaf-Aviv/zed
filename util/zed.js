...
const ddragonVer   = '8.14.1'
const ddragon      = `//ddragon.leagueoflegends.com/cdn/8.14.1`
...
async function makeSpecBatch(specObject) {
    const specBat = String.raw`CD /D C:\Riot Games\League of Legends\RADS\solutions\lol_game_client_sln\releases\0.0.1.217\deploy`
    ...
    return redisClient.getAsync(`summoner_${summonerName}_${region}`)
        .then(async summonerData => {
            ...
            summonerData = await rp({
                uri: encodeURI(`https://${region}${riot}summoner/v3/summoners/by-name/${summonerName}?api_key=${process.env.LOL_KEY}`),
                json: true
            })
...
async function getMatchSummary(gameId, region) {
    let summary = await Match.findOne({ gameId })
    if (summary) return summary.toObject()
    summary = await rp({
...
async function getLeague(leagueId, region) {
    return redisClient.getAsync(`league_${leagueId}`)
        .then(async league => {
            ...
            if (league) redisClient.SET(`league_${leagueId}`, JSON.stringify(league), 'EX', 3600 * 12)
            return league 
        })
}
...
// STATIC CHAMPION STATS
async function getStaticChampStats(champName) {
    champName = champName[0].toUpperCase() + champName.slice(1).replace(/'/g, '').replace(/ /g, '')
    const champStats = await rp({
        uri: `http://ddragon.leagueoflegends.com/cdn/${ddragonVer}/data/en_US/champion/${champName}.json`,
        json: true
    })
    .catch(err => console.log('getStaticChampStats ERR:', err))
    return champStats.data[champName]
}
...
async function getIndepthStats(champName, position, elo) {
    ...
    if (position) {
        ...
        return indepthStats.filter(champ => champ.role == position)
    } else {
        return [indepthStats
...
function getChampDesc(champ) {
    champ = Object.keys(championIds).filter(k => championIds[k].toLowerCase() === champ.toLowerCase())
